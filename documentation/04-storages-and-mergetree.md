...
| `<col>.mrk2` / `.cmrk2` | "Mark" files: byte offsets into `.bin` for each granule's start. |
...
- **Merges** (`MergeTask`). The merge selector ([src/Storages/MergeTree/MergeSelectors/](../src/Storages/MergeTree/MergeSelectors/)) periodically picks adjacent parts to merge based on size/age heuristics, producing larger sorted parts and shrinking the active set. For `*ReplacingMergeTree` / `*SummingMergeTree` / etc., this is also where row-level dedup/aggregation happens.
...
`ALTER TABLE ... DELETE / UPDATE` is `mutate()` ([src/Storages/MergeTree/MutateTask.h](../src/Storages/MergeTree/MutateTask.h)). The mutation is registered (`mutations` table in `system.mutations` shows progress) and each part gets rewritten with the mutation's effect applied. Until rewrite, reads transparently apply the predicate. `ALTER TABLE ... DELETE WHERE` therefore is **eventually consistent within the replica**, not instantaneous — though "lightweight deletes" (a marker column variant) hide that behind a faster path.
